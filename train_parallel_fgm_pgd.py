...
from model import BERTLinearModel,BERTLinearModel_NER, BERTLinearModel_SA
import argparse
import os
...
#               'hlf/rbtl3']
args = parse()
writer = SummaryWriter('/home/zoulexiao/workspace/nlp_contest/log/'+args.experiment_name)
...
for i in trange(0, int(epoch), desc="Epoch", disable=False):
    ...
    for step, batch in enumerate(iter_bar):
        ...
        loss.backward()
#        fgm.attack()
#        loss_adv = loss_func(model(input_ids = input_ids,
#                                      attention_mask = attention_mask,
#                                      token_type_ids = token_type_ids,
#                                      position_ids = position_ids), classes)
#        loss_adv.backward()
#        fgm.restore()
        pgd.backup_grad()
        for t in range(K):
